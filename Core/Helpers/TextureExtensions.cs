...
public static unsafe class TextureExtensions
{
    public static void WriteImage(this Texture2D texture, string file)
    {
        ...
        {
            texture.FlushTexture(ptr, new Vector2D<uint>((uint)image.Width, (uint)image.Height), GLEnum.Rgba, GLEnum.UnsignedByte);
        }
    }
    ...
    public static void WriteImage(this Texture3D texture, GLEnum target, string file)
    {
        ...
        fixed (byte* ptr = image.Data)
        {
            texture.FlushTexture(ptr, new Vector2D<uint>((uint)image.Width, (uint)image.Height), target, GLEnum.Rgba, GLEnum.UnsignedByte);
        }
    }
    ...
    public static void WriteColor(this Texture3D texture, GLEnum target, Vector4D<byte> color)
    {
        ...
        fixed (byte* ptr = bytes)
        {
            ...
            texture.FlushTexture(ptr, new Vector2D<uint>(1, 1), target, GLEnum.Rgba, GLEnum.UnsignedByte);
        }
    }
}
